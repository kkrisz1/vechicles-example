...
public class VehicleDaoImpl implements VehicleDao {
    ...
    @Override
    public Vehicles getVehicleWithinCircle(double longitude, double latitude, double radius) {
        final double angle = radius / RADIUS_EARTH;
        final double centerRadLong = Math.toRadians(longitude);
        final double centerRadLat = Math.toRadians(latitude);

        return vehicleMap.values().parallelStream()
                .filter(vehicle -> Objects.nonNull(vehicle.getLocation()))
                .filter(vehicle -> {
                    final double latMin = centerRadLat - angle;
                    final double latMax = centerRadLat + angle;
                    final double radLat = Math.toRadians(vehicle.getLocation().getLatitude());
                    return radLat >= latMin && radLat <= latMax;
                })
                .filter(vehicle -> {
                    final double lngDelta = Math.asin(Math.sin(angle) * Math.cos(centerRadLat));
                    final double radLng = Math.toRadians(vehicle.getLocation().getLongitude());
                ...
                .filter(vehicle -> {
                    ...
                    double radLong2 = Math.toRadians(vehicle.getLocation().getLongitude());
                    double radLat2 = Math.toRadians(vehicle.getLocation().getLatitude());
